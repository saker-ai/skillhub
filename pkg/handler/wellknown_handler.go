...
func renderInstallMarkdown(base string) string {
	...
	b.WriteString("curl -fsS \"" + base + "/api/v1/skills/<slug>/versions\"\n")
	b.WriteString("curl -fsS \"" + base + "/api/v1/skills/<slug>/versions/1.2.3\"\n")
	b.WriteString("```\n\n")
	b.WriteString("### Read a single file inside a skill (default `SKILL.md`)\n\n")
	b.WriteString("```bash\n")
	b.WriteString("curl -fsS \"" + base + "/api/v1/skills/<slug>/file?version=latest&path=SKILL.md\"\n")
	b.WriteString("```\n\n")
	...
	b.WriteString("Optional fields: `displayName`, `summary`, `category`, `tags` (comma-separated), `kind`, `visibility` (`public|private`), `namespace`, `changelog`, `dependencies` (JSON array of `{slug,versionRange}`), `signature` (cosign bundle, ≤ 64 KB).\n\n")
	b.WriteString("```bash\n")
	...
	b.WriteString("- `409` — conflict (e.g. namespace already at the team-token quota; revoke an unused one before retrying)\n")
